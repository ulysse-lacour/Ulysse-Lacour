...
# Update template
echo -e "\n${VIOLET}${BOLD}Extracting $PROJECT_NAME production template to locale $TEMPLATE_DIR directory :${RESET}\n"
expect <<EOF
    set timeout -1
...
if [ $? -eq 0 ]; then
    echo -e "\n${SUCCESS}Production template extracted successfully to ./directus/$TEMPLATE_DIR!\n"
    echo -e "${DEFAULT}Local Directus app template can be updated with ${YELLOW}pnpm apply-template${RESET}\n"
else
    handle_error
fi
