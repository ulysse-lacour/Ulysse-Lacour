...
#===========================================
# Scripts to start use directus-sync PUSH function
# Version: 1.0
#===========================================
...
# Push configuration to Directus
push_config() {
    print_header "Push configuration"
    print_message "info" "Pushing..."

    DIRECTUS_CONTAINER_NAME="${PROJECT_NAME}__directus"
    docker exec -i $DIRECTUS_CONTAINER_NAME npx directus-sync push -u "${DIRECTUS_URL}" -t "${DIRECTUS_ADMIN_TOKEN}"

    if [ $? -eq 0 ]; then
        print_message "success" "Config successfully pushed"
    else
        print_message "error" "Failed to push config"
        exit 1
    fi
}
